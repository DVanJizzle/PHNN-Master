...
def get_args():
    ...
    parser.add_argument('--learn_rate', default=9e-4, type=float, help='learning rate')
    ...
    parser.add_argument('--seed', default=2, type=int, help='randomness seed')
    parser.add_argument('--batch_size', default=128, type=int, help='size of batches for training')
    parser.add_argument('--perm', default=[2, 0, 1], type=list, help='Setup of the dissipation matrix. 2 is quadratic, 1 linear, 0 constant')
...
def train(args):
    ...
    while True:
        ...
        for index, val in enumerate(batch_numbers):
            x_output_nn = odeint(func=chi, y0=x_input[val], t=torch.linspace(t[val % (len(t) - 1)], t[val % (len(t) - 1) + 1], args.integration_steps), rtol=1e-10, method='implicit_midpoint')[-1]  # Midpoint is A-stable isometry t[[index%29, index%29+1]]
...
def fun_factory(J, G, model_hamiltonian, model_R, input_dim):
    def chi(t, x):
        #u = getU(t).unsqueeze(0)
        L_upper_triangle = torch.zeros(2*input_dim, 2*input_dim)
        L_result = model_R(x)
        position = 0
        for i in range(2*input_dim):
            for j in range(2*input_dim):
                if j < i:
                    continue
                L_upper_triangle[i, j] = L_result[position]
                position = position + 1
