...
def hamiltonian(qp):
    ...
    H = 0.5 * p**2 + 1 - math.cos(q)
    return H


def gradient_hamiltonian(q, p):
    return torch.tensor([math.sin(q), p])
...
def get_trajectory(t_span=[0, 6], timescale=5):
    ...
    # get initial value
    x0 = np.zeros(2)
    x0[0] = np.random.rand() * 2*math.pi - math.pi
    Hmax = math.sqrt(2 + 2*math.cos(x0[0]))
    x0[1] = np.random.rand() * 2*Hmax - Hmax
    x0 = torch.tensor(x0)
...
def get_dataset(seed=0, samples=100, t_span=[0, 6], timescale=20, noise=0.):
    data = {'meta': locals()}
    np.random.seed(seed)
